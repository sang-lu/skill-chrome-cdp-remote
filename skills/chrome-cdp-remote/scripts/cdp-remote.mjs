#!/usr/bin/env node
// cdp-remote - lightweight Chrome DevTools Protocol CLI for a REMOTE Chrome
// Uses raw CDP over WebSocket, no Puppeteer dependency.
// Requires Node 22+ (built-in WebSocket + fetch).
//
// Unlike chrome-cdp (which auto-discovers a LOCAL browser's DevToolsActivePort
// file), this connects to a remote Chrome that already has CDP listening at
// a known host:port. Configure the endpoint with CDP_URL (or CDP_HOST/CDP_PORT).
//
// Per-tab persistent daemon: page commands go through a daemon that holds the
// CDP session open, avoiding a fresh WebSocket handshake (costly over a WAN)
// on every command. Daemons auto-exit after 20min idle.
...
// ---------------------------------------------------------------------------
// Remote endpoint configuration (env-var only, so it's resolved synchronously
// and consistently before any daemon/socket paths are derived from it)
// ---------------------------------------------------------------------------

function endpointConfigString() {
  const url = process.env.CDP_URL;
  if (url) return url;
  const host = process.env.CDP_HOST;
  if (host) return `${host}:${process.env.CDP_PORT || '9222'}`;
  return null;
}

function endpointKey() {
  const cfg = endpointConfigString();
  if (!cfg) return 'unconfigured';
  return createHash('sha1').update(cfg).digest('hex').slice(0, 10);
}

const RUNTIME_ROOT = IS_WINDOWS
  ? resolve(process.env.LOCALAPPDATA || resolve(homedir(), 'AppData', 'Local'), 'cdp-remote')
  : process.env.XDG_RUNTIME_DIR
    ? resolve(process.env.XDG_RUNTIME_DIR, 'cdp-remote')
    : resolve(homedir(), '.cache', 'cdp-remote');
const RUNTIME_DIR = resolve(RUNTIME_ROOT, endpointKey());
...
function sockPath(targetId) {
  return IS_WINDOWS
    ? `\\\\.\\pipe\\cdp-remote-${endpointKey()}-${targetId}`
    : resolve(RUNTIME_DIR, `cdp-${targetId}.sock`);
}

// Fetch the browser-level webSocketDebuggerUrl from a remote Chrome's
// /json/version endpoint, and normalize it to point at the host:port we
// actually reached (Chrome sometimes reports "localhost" in the URL even
// when bound to 0.0.0.0, which is unreachable from outside that machine).
async function getWsUrl() {
  const cfg = endpointConfigString();
  if (!cfg) {
    throw new Error(
      'No remote CDP endpoint configured. Set CDP_URL (e.g. CDP_URL=http://192.168.1.50:9222) ' +
      'or CDP_HOST (+ optional CDP_PORT, default 9222) before running this command.'
    );
  }

  let base = cfg;
  if (!/^https?:\/\//.test(base) && !/^wss?:\/\//.test(base)) base = `http://${base}`;

  // A direct browser-level ws:// URL was given — use as-is.
  ...
  try {
    ...
    const res = await fetch(`${httpBase}/json/version`, { signal: controller.signal });
    clearTimeout(timer);
    if (!res.ok) throw new Error(`HTTP ${res.status}`);
    versionInfo = await res.json();
  } catch (e) {
    throw new Error(
      `Cannot reach CDP endpoint at ${httpBase}/json/version (${e.message}). Checklist:\n` +
      `  - Remote Chrome running with --remote-debugging-port and --remote-debugging-address=0.0.0.0?\n` +
      `  - Chrome 111+: also needs --remote-allow-origins=* (or a matching origin) or it silently rejects the WebSocket.\n` +
      `  - Port reachable from here? Try: curl ${httpBase}/json/version\n` +
      `  - If the port isn't exposed publicly, tunnel it: ssh -L 9222:localhost:9222 user@remote-host, then use CDP_URL=http://127.0.0.1:9222`
    );
  }

  let wsUrl = versionInfo.webSocketDebuggerUrl;
  if (!wsUrl) throw new Error(`No webSocketDebuggerUrl in response from ${httpBase}/json/version`);

  try {
    const reqHost = new URL(httpBase).host; // host:port we actually reached
    const wsParsed = new URL(wsUrl);
    wsParsed.host = reqHost;
...
class CDP {
  ...
  async connect(wsUrl) {
    return new Promise((res, rej) => {
      ...
      const timer = setTimeout(() => rej(new Error(`Timed out connecting to ${wsUrl}`)), CONNECT_TIMEOUT);
...
async function runDaemon(targetId) {
  ...
  try {
    ...
    process.stderr.write(`Daemon: cannot connect to remote Chrome: ${e.message}\n`);
    process.exit(1);
  }
...
async function getOrStartTabDaemon(targetId) {
  ...
  const child = spawn(process.execPath, [process.argv[1], '_daemon', targetId], {
    detached: true,
    stdio: 'ignore',
    env: process.env, // carry CDP_URL / CDP_HOST / CDP_PORT into the daemon
  });
  child.unref();
  ...
  throw new Error('Daemon failed to start — check that the remote CDP endpoint is still reachable (run "list" for diagnostics).');
}
...
const USAGE = `cdp-remote - lightweight Chrome DevTools Protocol CLI for a REMOTE Chrome

Configure the endpoint via environment variables (set once per shell, or
prefix each command):
  CDP_URL=http://192.168.1.50:9222 scripts/cdp-remote.mjs list
  CDP_HOST=192.168.1.50 CDP_PORT=9222 scripts/cdp-remote.mjs list   # PORT defaults to 9222
  CDP_URL can also be a direct browser ws:// endpoint (ws://host:port/devtools/browser/<id>)

Usage: cdp-remote.mjs <command> [args]
...
EVAL SAFETY NOTE
  ...
  eval calls when the list can change between calls. Prefer stable selectors
  or collect all data in a single eval.

MULTIPLE REMOTE ENDPOINTS
  Daemons and the page cache are namespaced per CDP_URL/CDP_HOST:CDP_PORT, so
  you can point this CLI at different remote Chromes across invocations
  without their sessions colliding.

DAEMON IPC (for advanced use / scripting)
  Each tab runs a persistent daemon at a Unix socket in the runtime dir.
